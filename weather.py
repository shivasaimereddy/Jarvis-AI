...
def weather():
    ...
    if data_json['cod'] == 200:
        ...
        print('Current location is ' +
              data_json['name'] + data_json['sys']['country'] + 'dia')
        speak('weather type is ' + weather_desc['main'])
        print('weather type ' + weather_desc['main'])
        speak('Wind speed is at ' + str(wind['speed']) + ' metre per second')
        print('Wind speed is at ' + str(wind['speed']) + ' metre per second')
        speak('Current Temperature is: ' +
              str(main['temp']) + 'degree celcius')
